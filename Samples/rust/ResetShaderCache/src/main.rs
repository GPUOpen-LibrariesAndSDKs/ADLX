...
//! Demonstrates how to reset the GPU shader cache with ADLX. This is the Rust
//! counterpart of `Samples/C/3DGraphics/ResetShaderCache`.
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DResetShaderCache = std::ptr::null_mut();
                if adlx_succeeded((svc_vtbl.GetResetShaderCache)(svc, gpu, &mut feature)) {
                    let f = &*(*feature).pVtbl;
                    println!("\n  === Reset Shader Cache ===");
                    ...
                    if adlx_succeeded((f.IsSupported)(feature, &mut supported)) {
                        ...
                        if supported != 0 {
                            let res = (f.ResetShaderCache)(feature);
                            println!("\tReset shader cache result: {} (0 means success)", res);
                        }
                    }
                    (f.Release)(feature);
                } else {
                    println!("Failed to get Reset Shader Cache interface");
                }
