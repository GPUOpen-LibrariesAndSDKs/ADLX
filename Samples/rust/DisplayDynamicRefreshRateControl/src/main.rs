...
//! Demonstrates how to query Dynamic Refresh Rate Control per display with ADLX.
//! This is the Rust counterpart of `Samples/C/Display/DisplayDynamicRefreshRateControl`.
//!
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === Dynamic Refresh Rate Control on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayDynamicRefreshRateControl = std::ptr::null_mut();
                    if adlx_succeeded((ds3_vtbl.GetDynamicRefreshRateControl)(ds3, display, &mut feature))
                    {
                        let f = &*(*feature).pVtbl;
