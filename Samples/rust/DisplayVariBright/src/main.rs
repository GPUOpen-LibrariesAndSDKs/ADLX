...
//! Demonstrates how to query AMD VariBright per display with ADLX. This is the Rust
//! counterpart of `Samples/C/Display/DisplayVariBright`.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === VariBright on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayVariBright = std::ptr::null_mut();
                    if adlx_succeeded((ds_vtbl.GetVariBright)(display_service, display, &mut feature))
                    {
                        let f = &*(*feature).pVtbl;
                        ...
                        let modes: &[(&str, unsafe extern "system" fn(
                            *mut IADLXDisplayVariBright,
                            *mut adlx_bool,
                        ) -> ADLX_RESULT)] = &[
                            ("Maximize brightness", f.IsCurrentMaximizeBrightness),
                            ("Optimize brightness", f.IsCurrentOptimizeBrightness),
                            ("Balanced", f.IsCurrentBalanced),
                            ("Optimize battery", f.IsCurrentOptimizeBattery),
                            ("Maximize battery", f.IsCurrentMaximizeBattery),
                        ];
                        for (label, getter) in modes {
                            let mut s: adlx_bool = 0;
                            if adlx_succeeded(getter(feature, &mut s)) && s != 0 {
                                println!("\tCurrent mode: {}", label);
                            }
                        }
