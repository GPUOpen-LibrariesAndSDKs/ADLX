...
//! Demonstrates how to query anti-aliasing settings with ADLX. This is the Rust
//! counterpart of `Samples/C/3DGraphics/AntiAliasing`.
//!
//! Mode, level, and method are ADLX enums; their integer values are printed here
//! (see ADLXDefines.h for the value meanings).
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DAntiAliasing = std::ptr::null_mut();
                if adlx_succeeded((svc_vtbl.GetAntiAliasing)(svc, gpu, &mut feature)) {
                    let f = &*(*feature).pVtbl;
                    println!("\n  === Anti-Aliasing ===");
                    ...
                    if adlx_succeeded((f.GetMethod)(feature, &mut method)) {
                        println!("\tCurrent method (enum value): {}", method);
                    }
                    (f.Release)(feature);
                } else {
                    println!("Failed to get Anti-Aliasing interface");
                }
