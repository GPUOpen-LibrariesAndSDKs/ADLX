...
//! Demonstrates how to query HDCP per display with ADLX. This is the Rust
//! counterpart of `Samples/C/Display/DisplayHDCP`.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === HDCP on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayHDCP = std::ptr::null_mut();
                    if adlx_succeeded((ds_vtbl.GetHDCP)(display_service, display, &mut feature)) {
                        let f = &*(*feature).pVtbl;
