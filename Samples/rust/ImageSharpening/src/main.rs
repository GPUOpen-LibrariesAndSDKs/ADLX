...
//! Demonstrates how to query AMD Radeon Image Sharpening with ADLX. This is the Rust
//! counterpart of `Samples/C/3DGraphics/ImageSharpening`.
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DImageSharpening = std::ptr::null_mut();
                if adlx_succeeded((svc_vtbl.GetImageSharpening)(svc, gpu, &mut feature)) {
                    let f = &*(*feature).pVtbl;
                    println!("\n  === Image Sharpening ===");
                    ...
                    println!("Failed to get Image Sharpening interface");
                }
