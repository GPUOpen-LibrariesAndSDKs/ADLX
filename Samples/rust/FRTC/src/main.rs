...
//! Demonstrates how to query Frame Rate Target Control (FRTC) with ADLX. This is the
//! Rust counterpart of `Samples/C/3DGraphics/FRTC`.
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DFrameRateTargetControl = std::ptr::null_mut();
                if adlx_succeeded((svc_vtbl.GetFrameRateTargetControl)(svc, gpu, &mut feature)) {
                    let f = &*(*feature).pVtbl;
                    ...
                    let mut fps: adlx_int = 0;
                    if adlx_succeeded((f.GetFPS)(feature, &mut fps)) {
                        println!("\tCurrent target FPS: {}", fps);
                    }
                    (f.Release)(feature);
                } else {
                    println!("Failed to get FRTC interface");
                }
