...
//! Demonstrates how to query the display color depth with ADLX. This is the Rust
//! counterpart of `Samples/C/Display/DisplayColorDepth`.
//!
//! The current color depth is an `ADLX_COLOR_DEPTH` enum; its integer value is
//! printed along with the per-BPC support flags.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === Color depth on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayColorDepth = std::ptr::null_mut();
                    if adlx_succeeded((ds_vtbl.GetColorDepth)(display_service, display, &mut feature))
                    {
                        let f = &*(*feature).pVtbl;
                        ...
                        if adlx_succeeded((f.GetValue)(feature, &mut value)) {
                            println!("\tCurrent color depth (enum value): {}", value);
                        }

                        let bpc: &[(&str, unsafe extern "system" fn(
                            *mut IADLXDisplayColorDepth,
                            *mut adlx_bool,
                        ) -> ADLX_RESULT)] = &[
                            ("6", f.IsSupportedBPC_6),
                            ("8", f.IsSupportedBPC_8),
                            ("10", f.IsSupportedBPC_10),
                            ("12", f.IsSupportedBPC_12),
                            ("14", f.IsSupportedBPC_14),
                            ("16", f.IsSupportedBPC_16),
                        ];
                        for (label, getter) in bpc {
                            let mut s: adlx_bool = 0;
                            if adlx_succeeded(getter(feature, &mut s)) {
                                println!("\tBPC {} supported: {}", label, s != 0);
                            }
                        }
