...
//! Demonstrates how to query AMD Radeon Chill with ADLX. This is the Rust
//! counterpart of `Samples/C/3DGraphics/Chill`.
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DChill = std::ptr::null_mut();
                if adlx_succeeded((svc_vtbl.GetChill)(svc, gpu, &mut feature)) {
                    let f = &*(*feature).pVtbl;
                    println!("\n  === Radeon Chill ===");
                    ...
                    if adlx_succeeded((f.GetFPSRange)(feature, &mut range)) {
                        println!("\tFPS range: {} .. {}", range.minValue, range.maxValue);
                    }
                    let mut min_fps: adlx_int = 0;
                    if adlx_succeeded((f.GetMinFPS)(feature, &mut min_fps)) {
                        println!("\tCurrent min FPS: {}", min_fps);
                    }
                    let mut max_fps: adlx_int = 0;
                    if adlx_succeeded((f.GetMaxFPS)(feature, &mut max_fps)) {
                        println!("\tCurrent max FPS: {}", max_fps);
                    }
                    (f.Release)(feature);
                } else {
                    println!("Failed to get Chill interface");
                }
