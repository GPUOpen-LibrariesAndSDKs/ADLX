...
//! Demonstrates how to query Morphological Anti-Aliasing with ADLX. This is the Rust
//! counterpart of `Samples/C/3DGraphics/MorphologicAntiAliasing`.
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DMorphologicalAntiAliasing = std::ptr::null_mut();
                if adlx_succeeded(
                    (svc_vtbl.GetMorphologicalAntiAliasing)(svc, gpu, &mut feature),
                ) {
                    let f = &*(*feature).pVtbl;
                    ...
                    println!("Failed to get Morphological Anti-Aliasing interface");
                }
