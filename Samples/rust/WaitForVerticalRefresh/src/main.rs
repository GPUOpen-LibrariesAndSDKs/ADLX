...
//! Demonstrates how to query the Wait for Vertical Refresh (VSync) setting with ADLX.
//! This is the Rust counterpart of `Samples/C/3DGraphics/WaitForVerticalRefresh`.
//!
//! The mode is an `ADLX_WAIT_FOR_VERTICAL_REFRESH_MODE` enum; its integer value is
//! printed here (see ADLXDefines.h for the value meanings).
    ...
    unsafe {
        ...
        if adlx_succeeded((sys_vtbl.GetGPUs)(sys, &mut gpus)) {
            ...
            if adlx_succeeded((list_vtbl.At_GPUList)(gpus, (list_vtbl.Begin)(gpus), &mut gpu)) {
                let mut feature: *mut IADLX3DWaitForVerticalRefresh = std::ptr::null_mut();
                if adlx_succeeded(
                    (svc_vtbl.GetWaitForVerticalRefresh)(svc, gpu, &mut feature),
                ) {
                    let f = &*(*feature).pVtbl;
                    ...
                    println!("Failed to get Wait for Vertical Refresh interface");
                }
