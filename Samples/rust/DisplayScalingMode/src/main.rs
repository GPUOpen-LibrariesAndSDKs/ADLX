...
//! Demonstrates how to query the display scaling mode with ADLX. This is the Rust
//! counterpart of `Samples/C/Display/DisplayScalingMode`.
//!
//! The scaling mode is an `ADLX_SCALE_MODE` enum; its integer value is printed here
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === Scaling mode on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayScalingMode = std::ptr::null_mut();
                    if adlx_succeeded((ds_vtbl.GetScalingMode)(display_service, display, &mut feature))
                    {
                        let f = &*(*feature).pVtbl;
