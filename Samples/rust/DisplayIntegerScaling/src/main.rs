...
//! Demonstrates how to query integer scaling per display with ADLX. This is the Rust
//! counterpart of `Samples/C/Display/DisplayIntegerScaling`.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === Integer scaling on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayIntegerScaling = std::ptr::null_mut();
                    if adlx_succeeded(
                        (ds_vtbl.GetIntegerScaling)(display_service, display, &mut feature),
                    ) {
                        let f = &*(*feature).pVtbl;
