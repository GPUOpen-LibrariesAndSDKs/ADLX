...
//! Demonstrates how to query Virtual Super Resolution (VSR) per display with ADLX.
//! This is the Rust counterpart of `Samples/C/Display/DisplayVSR`.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === VSR on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayVSR = std::ptr::null_mut();
                    if adlx_succeeded(
                        (ds_vtbl.GetVirtualSuperResolution)(display_service, display, &mut feature),
                    ) {
                        let f = &*(*feature).pVtbl;
