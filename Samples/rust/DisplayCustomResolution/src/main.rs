...
//! Demonstrates how to query custom resolution support per display with ADLX. This
//! is the Rust counterpart of `Samples/C/Display/DisplayCustomResolution`.
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === Custom resolution on '{}' ===", display_name(display));

                    let mut cr: *mut IADLXDisplayCustomResolution = std::ptr::null_mut();
                    if adlx_succeeded((ds_vtbl.GetCustomResolution)(display_service, display, &mut cr))
                    {
                        let v = &*(*cr).pVtbl;
                        let mut supported: adlx_bool = 0;
                        if adlx_succeeded((v.IsSupported)(cr, &mut supported)) {
