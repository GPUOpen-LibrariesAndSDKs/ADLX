...
//! Demonstrates how to query FreeSync Color Accuracy per display with ADLX. This is
//! the Rust counterpart of `Samples/C/Display/DisplayFreeSyncColorAccuracy`.
//!
        ...
        if adlx_succeeded((ds_vtbl.GetDisplays)(display_service, &mut display_list)) {
            ...
            while it != end {
                ...
                if adlx_succeeded((dl_vtbl.At_DisplayList)(display_list, it, &mut display)) {
                    println!("\n  === FreeSync Color Accuracy on '{}' ===", display_name(display));

                    let mut feature: *mut IADLXDisplayFreeSyncColorAccuracy = std::ptr::null_mut();
                    if adlx_succeeded((ds3_vtbl.GetFreeSyncColorAccuracy)(ds3, display, &mut feature))
                    {
                        let f = &*(*feature).pVtbl;
