...
from amd_display_mcp_server.utils.log import log_with_color, init_log_configuration
...
def load_tools_manifest() -> List[Dict]:
    ...
    try:
        if os.path.exists(tools_manifest):
            ...
            log_with_color(f"Tools manifest not found at {tools_manifest}", "red")
            return []
...
async def serve(transport: str = "stdio") -> None:
    """Run the amd display MCP server with optimized performance.
    
    Args:
        transport: Communication transport mode - either 'stdio' or 'streamable_http'
    """
    server = Server("mcp-amd-display")
...
def main():
    """MCP AMD display Server entry point"""
    import argparse
    import asyncio

    parser = argparse.ArgumentParser(
        description="Give a model the ability to operate AMD display setting features.",
    )
