...
from amd_gputunings_mcp_server.utils.log import log_with_color, init_log_configuration
...
async def serve(transport: str = "stdio") -> None:
    """Run the GPU tunings MCP server with optimized performance.
    
    Args:
        transport: Communication transport mode - either 'stdio' or 'streamable_http'
    """
    server = Server("mcp-gpu-tunings")
...
def main():
    """MCP AMD GPU tunings Server entry point"""
    import argparse
    import asyncio
